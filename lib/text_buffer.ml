...
let swap_line target_str target buffer =
  ...
  let target_zipper =
    match target with
    | Start -> z
    | Curswant -> apply_n buffer.curswant Zipper.move_right z
    | End -> Zipper.move_end z
  in
...
let get_class style u : char_class =
  ...
  | _, (' ' | '\t') -> Space
  | WORD, _ -> Word
  | Word, c when is_alphanum c -> Word
  | Word, _ -> Other
...
let rec skip_while p f buffer =
  if p buffer then
    match f buffer with
    | None -> buffer
    | Some next_buffer -> skip_while p f next_buffer
  else buffer

let move_word style direction trajectory buffer : t =
  let next_char b =
    match direction with
    | Forward -> Zipper.focus b.current_line
    | Backward -> Zipper.focus_left b.current_line
  in
  ...
  let step, rev_step =
    match direction with
    | Forward -> (step_forward, step_backward)
    | Backward -> (step_backward, step_forward)
  in

  let buffer =
    match (direction, trajectory) with
    | Forward, End -> buffer |> step |> Option.value ~default:buffer
    (* TODO *)
    | _ -> buffer
  in
  ...
  let buffer =
    match (direction, trajectory) with
    | Forward, End -> buffer |> rev_step |> Option.value ~default:buffer
    | Backward, Start -> buffer |> step |> Option.value ~default:buffer
    | _ -> buffer
  in
